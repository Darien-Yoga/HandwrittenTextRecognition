...
class Model: 
	...
	def setupRNN(self):
		...
		num_units = [256, 256]
		i=0
		cells = [tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=n,state_is_tuple=True) for n in num_units]
		# stack basic cells
		stacked_rnn_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(cells, state_is_tuple=True)

		# bidirectional RNN
		# BxTxF -> BxTx2H	
		((fw, bw), _) = tf.nn.bidirectional_dynamic_rnn(cell_fw=stacked_rnn_cell, cell_bw=stacked_rnn_cell, inputs=rnnIn3d, dtype=rnnIn3d.dtype)
